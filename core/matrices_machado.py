"""
Matrices de simulacion de daltonismo 

Las claves de severidad van de 0.0 a 1.0 con paso 0.1:
- 0.0: vision normal / sin deficiencia.
- 1.0: maxima severidad, equivalente a dicromacia.

Tambien se permite ingresar la severidad como escala 0 a 10 desde el main:
0 -> 0.0, 1 -> 0.1, ..., 10 -> 1.0.
"""
...
NOMBRES_LEGIBLES = {
    "protanomaly": "Protanomalía/Protanopia",
    "deuteranomaly": "Deuteranomalía/Deuteranopia",
    "tritanomaly": "Tritanomalía/Tritanopia",
}
...
def normalizar_tipo(tipo: str) -> str:
    """Devuelve la clave interna del tipo de daltonismo."""
    ...
    if clave not in TIPOS_VALIDOS:
        raise ValueError(
            "Tipo de daltonismo invalido. Use protan, deutan/deuteran o tritan."
        )
...
def normalizar_severidad(valor) -> float:
    ...
    if severidad not in MATRICES_CVD["protanomaly"]:
        raise ValueError("La severidad debe corresponder a un paso de 0.1.")

    return severidad


def obtener_matriz(tipo: str, severidad=1.0) -> tuple[np.ndarray, str, float]:
    """Obtiene una copia de la matriz para el tipo y severidad solicitados."""
    tipo_normalizado = normalizar_tipo(tipo)
    severidad_normalizada = normalizar_severidad(severidad)
    matriz = np.array(
        MATRICES_CVD[tipo_normalizado][severidad_normalizada],
        dtype=np.float32,
    )
    return matriz, tipo_normalizado, severidad_normalizada
